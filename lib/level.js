...
const transform = path => {
  return new Transform({
    ...
    transform (chunk, enc, cb) {
      console.log(chunk)
      cb(null, JSON.stringify(chunk.key.slice(0, path.length)) === JSON.stringify(path) ? chunk : null)
    }
  })
}
...
exports.load = (id, { context, path }) => {
  ...
  return getDb(id)
    .then(db => new Promise((resolve, reject) => db.createReadStream().pipe(transform(path))))
}
